...
class TopProductComponent extends React.Component {
  ...
  componentDidMount() {
    let data = [];
    console.log(this.props.topproduct, "check 2");
  ...
  redirectToPDP = (item, bottleSize, childId) => {
    const bottleSizeDetail = _find(_get(item, "child"), [
      "bottle_size",
      bottleSize,
    ]);
  ...
  handlebottleSizeClick = (data, parentIndex, item, childIndex) => {
    ...
    this.redirectToPDP(
      ...
      _get(item, `child[${this.getActiveChildIndex(parentIndex)}].id`, "")
    );
  };
  render() {
    ...
    let galleryItems = _map(
      _get(this.props, "topproduct", []),
      (it, parentIndex) => {
        let smallImage = "";
        ...
        return (
          <React.Fragment key={parentIndex}>
            <div className="storeItemsList mb-5">
              <div
                onClick={() =>
                  this.redirectToPDP(
                    item,
                    _get(
                      item,
                      `child[${this.getActiveChildIndex(
                        parentIndex
                      )}].bottle_size`,
                      ""
                    ),
              ...
              >
                ...
                <div
                  style={{
                    backgroundImage: `url(${_get(item, "smallImage", "")})`,
                  }}
                ...
                <div className="productName">{_get(item, "name")}</div>
                {/* <div className="displaySize">
                                {_get(item, `child[${this.getActiveChildIndex(parentIndex)}].bottle_size`, '')}
                            </div> */}
                {_get(
                  item,
...
export default connect(mapStateToProps)(
  withStyles(styles)(TopProductComponent)
);
