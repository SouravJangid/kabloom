...
const axiosMiddleware = (store) => (next) => (action) => {
  ...
  const failureHandler =
    config.failure ||
    function (subreddit, error, errCode) {
      return {
        type: "DUMMY_ERROR",
        subreddit,
        error,
  ...
  if (config.body) requestObject.data = config.body;
  ...
  if (uploadConfig) requestObject.onUploadProgress = uploadConfig;
  ...
  axios(requestObject)
    .then((responseData) => {
      dispatch(
        successHandler(subreddit, responseData.data, id, resolve, constants)
      );
    })
    ...
    .catch((error) => {
      if (error.response) {
        ...
        return dispatch(
          failureHandler(subreddit, error, 500, reject, constants)
        );
      }
